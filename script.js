...
window.onload = function() {
  ...
  // Display Movies list
  let result = document.getElementById('movie').addEventListener("click" , getAttribute);
...
function getAttribute() {
  let sortAttr = document.getElementById('movie').value ;
  sortMoviesByAttr(movies , sortAttr) ;
  displayMovies(movies);
}
...
for (let j = 0; j < movies.length - 1; j++) {
  ...
  //     }
  // }
  let object = getMaxMovieObject(movies, j, sortAttr);
  // swap the first and the last
  movies[object.max_index] = movies[j] // --> 10
  movies[j] = object.max_number ;
}
